...
def preprocess_data(df: pd.DataFrame, null_threshold: float = 0.3) -> pd.DataFrame:
    ...
    if cols_to_drop:
        logger.info(f"Dropping columns with > {null_threshold*100}% nulls: {cols_to_drop}")
        df.drop(columns=cols_to_drop, inplace=True)
    else:
        logger.info("No columns dropped due to null threshold")
    ...
    for col in df.columns:
        if df[col].isnull().any():
            if df[col].dtype in ['float64', 'int64']:
                ...
                logger.info(f"Filled nulls in numeric column '{col}' with median: {fill_val}")
            else:
                fill_val = df[col].mode()[0]
                df[col].fillna(fill_val, inplace=True)
                logger.info(f"Filled nulls in categorical column '{col}' with mode: '{fill_val}'")
...
def main():
    try:
        ...
        logger.info(f"Head of processed data:\n{df_processed.head()}")

        if 'Resigned' in df_processed.columns:
            logger.info(f"Resigned value counts:\n{df_processed['Resigned'].value_counts()}")
        else:
            logger.warning("'Resigned' column not found in the dataset")
